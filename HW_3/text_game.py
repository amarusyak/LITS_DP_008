...
EXCEPTIONS = (NameError, SyntaxError, TypeError, ValueError)
...
def os_check():
    ...
    if os_type == 'Windows':
        ...
        raise EnvironmentError("Unsupported OS Type. Cannot continue.")
...
def numeric_choice(lower_limit=0, upper_limit=100):
    choice = None
...
def exams_passing(region):
    exams_pass_perc = EXAMS_PASS_PERC
    ...
    for n in range(1, 4):
        ...
        if exams_result < 100:
            print("Your result is {r} points...".format(r=exams_result))
        ...
        if exams_result >= EXAMS_PASS_PERC:
            print("Yey! You are lucky AF!") if region == 'America' \
                else print("Yey! Lucky you! :)")
            ...
            sleep(1)
            print("Just joking :) lol") if region == 'America' \
                else print("Just joking, never mind ;)")
            exams_pass_perc = 100
            break

    return exams_pass_perc


def main():
    os_check()
    welcome()
    global EXAMS_PASS_PERC

    # General variables:
    birth_location = random.choice(['America', 'Europe'])
    const_increase = 5
    universities = list()
    dead = None
    ...
    print("2) Dog")
    print("3) Cat")
    choice = numeric_choice(upper_limit=3)
    ...
    if choice is 1:
        ...
        if birth_location is 'America':
            ...
            if relationships == 'Y':
                ...
                print("Evaluate it in range from 1 ('don't really luw.') "
                      "to 50 ('FU***NG LOVE MY SPIRITUAL PERSON')")

                love = numeric_choice(upper_limit=50)
            ...
            exams_pass_perc = exams_passing(region='America')
            ...
            univer_choice = numeric_choice(upper_limit=len(universities))
            ...
            print("1) Boy")
            print("2) Girl")
            gender = numeric_choice(upper_limit=2)
            ...
            exams_pass_perc = exams_passing(region='Europe')
